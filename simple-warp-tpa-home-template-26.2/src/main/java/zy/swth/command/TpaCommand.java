...
public class TpaCommand {
    ...
    private static int executeTpa(CommandContext<CommandSourceStack> ctx) {
        ...
        try {
            ...
            source.sendFailure(Component.translatableWithFallback("swth.tpa.player_not_found", "玩家未找到"));
            return 0;
        }

        if (player.getUUID().equals(target.getUUID())) {
            source.sendFailure(Component.translatableWithFallback("swth.tpa.self", "不能向自己发送传送请求"));
            return 0;
        }
        ...
        player.sendSystemMessage(
                Component.translatableWithFallback("swth.tpa.sender_msg",
        ...
        Component acceptBtn = Component.translatableWithFallback("swth.tpa.accept_btn", " [接受] ")
                .withStyle(style -> style
                        .withColor(ChatFormatting.GREEN)
                        .withBold(true)
                        .withClickEvent(new ClickEvent.RunCommand("/tpay"))
                        .withHoverEvent(new HoverEvent.ShowText(
                                Component.translatableWithFallback("swth.tpa.accept_hover", "点击接受传送"))));

        Component denyBtn = Component.translatableWithFallback("swth.tpa.deny_btn", " [拒绝] ")
                .withStyle(style -> style
                        .withColor(ChatFormatting.RED)
                        .withBold(true)
                        .withClickEvent(new ClickEvent.RunCommand("/tpan"))
                        .withHoverEvent(new HoverEvent.ShowText(
                                Component.translatableWithFallback("swth.tpa.deny_hover", "点击拒绝传送"))));

        target.sendSystemMessage(
                Component.translatableWithFallback("swth.tpa.receiver_msg",
                        "%s 请求传送到你的位置 %s %s",
    ...
    private static int executeTpay(CommandContext<CommandSourceStack> ctx) {
        ...
        if (req == null) {
            source.sendFailure(Component.translatableWithFallback("swth.tpa.no_request", "没有待处理的传送请求"));
            return 0;
        }

        ServerPlayer requester = player.level().getServer().getPlayerList().getPlayer(req.requesterId());
        if (requester == null) {
            source.sendFailure(Component.translatableWithFallback("swth.tpa.requester_offline", "请求者已离线"));
            return 0;
        }
    ...
    private static int executeTpan(CommandContext<CommandSourceStack> ctx) {
        ...
        if (requester != null) {
            Component playerName = player.getDisplayName().copy().withStyle(ChatFormatting.AQUA);
            requester.sendSystemMessage(
                    Component.translatableWithFallback("swth.tpa.denied_sender",
        ...
        player.sendSystemMessage(
                Component.translatableWithFallback("swth.tpa.denied_receiver",
